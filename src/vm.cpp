...
StackFrame::StackFrame(const std::string name, int offset, int *instruction_pointer) : function_name(name), stack_start_offset(offset), instruction_pointer_offset(instruction_pointer)
{
}

StackFrame *StackFramePool::allocate(const std::string &name, size_t offset, int *instruction_p_offset)
{
	if (!pool.empty())
	{
...
void StackFramePool::deallocate(StackFrame *frame)
{
	frame->~StackFrame();
...
bool VM::checkStackFrameOverflow()
{
...
InterpretResult VM::run_computedGoTo()
{
	...
	DEFINE_GLOBAL:
	{
		std::string_view name = chunk->constants[*(instruction_pointer + 1)].returnStringView();
		vm_globals[std::string(name)] = stack.back();
		stack.pop_back();
		instruction_pointer += 2;
	}
		DISPATCH();
	...
	GET_GLOBAL:
	{
		...
		try
		{
			stack.emplace_back(vm_globals[std::string(name)]);
		}
	...
	CALL:
	{
		...
		std::string_view name_function = this->chunk->constants[offset].returnStringView();
		if (chunk->function.funcName == std::string(name_function))
		{
			isRecursive = 1;
		}
		else
		{
			isRecursive = 0;
			if (vm_native_functions.count(std::string(name_function)) == 0)
			{
				isNativeFn = 0;
			}
			else
		...
		if (isNativeFn)
		{
			NativeFn function = vm_native_functions.at(std::string(name_function)).function;
			Value *arguments = stack.size() == 0 ? NULL : &stack.back() - vm_native_functions.at(std::string(name_function)).arguments + 1;
			stack.emplace_back(function(vm_native_functions.at(std::string(name_function)).arguments, arguments));
			instruction_pointer += 2;
		}
		else
		{
			if (!isRecursive)
			{
				this->chunk = vm_functions[std::string(name_function)].get();
				arity = vm_functions[std::string(name_function)].get()->function.arity;
			}
